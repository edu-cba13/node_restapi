...
const ClinciasAltaComplejidad = require('./clinicas_alta_complejidad.models');
...
const load = async(req, res, next, id) => {
    try {
        req.rowResult = await ClinciasAltaComplejidad.findOne({
            where: { id: id },
            include: [{
...
const list = async(req, res, next) => {
    try {
        ...
        let resultado = await ClinciasAltaComplejidad.findAndCountAll({
            offset: offset,
            limit: limit,
...
const post = async(req, res, next) => {
    try {
        let clinicas = await ClinciasAltaComplejidad.create(req.body, {
            include: [{
                model: Domicilios,
                as: 'domicilios',
                include: [{
...
const put = async(req, res, next) => {
    try {
        if (req.rowResult.count === 0) {
            ...
            let resultado = await ClinciasAltaComplejidad.update({
                id_especialidad_clinica_alta_complejidad: req.body.id_especialidad_clinica_alta_complejidad,
...
const deleted = async(req, res, next) => {
    try {
        let resultado = await ClinciasAltaComplejidad.destroy({
            where: {
                id: req.params.id,
                estado_ioma: true
            }
...
const getClinicaAltaComplejidadAndDomicilios = async(req, res, next) => {
    try {
        let limit = req.query.limit || config.limitDefault;
        let offset = req.query.offset || config.offsetDefault;
        let resultado = await ClinciasAltaComplejidad.findAndCountAll({
...
module.exports = {
    ...
    put,
    deleted,
    getClinicaAltaComplejidadAndDomicilios
}